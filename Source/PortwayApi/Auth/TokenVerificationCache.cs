...
/// <summary>
/// Singleton cache for token verification results. Reduces per-request PBKDF2 hashing and SQLite round-trips from 3 to 0 on cache hit (30s TTL). Supports explicit invalidation by token ID on revocation
/// </summary>
public sealed class TokenVerificationCache(IMemoryCache cache) : ITokenVerificationCache
{
    ...
    /// <summary>
    /// Builds a stable, fixed-length cache key from the raw bearer token using SHA-256. The raw token is never stored; only its hash is used as the key. Uses a stack buffer for tokens ≤256 UTF-8 bytes, ArrayPool otherwise
    /// </summary>
