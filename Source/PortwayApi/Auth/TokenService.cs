...
public class TokenService
{
    ...
    /// <summary>
    /// Revoke (soft-delete / archive) a token by ID. Returns false when the token is not found or is protected by the last-token guard
    /// </summary>
    ...
    /// <summary>
    /// True when applying these values to this token would leave no full-access token active
    /// </summary>
    private async Task<bool> WouldRemoveLastFullAccessTokenAsync(int tokenId, string? newScopes, string? newEnvironments)
    {
        var active = await _dbContext.Tokens
            .AsNoTracking()
            .Where(t => t.RevokedAt == null && (t.ExpiresAt == null || t.ExpiresAt > DateTime.UtcNow))
        ...
        var fullAccess = active.Where(t => t.AllowedScopes == "*" && t.AllowedEnvironments == "*").ToList();
        if (fullAccess.Count != 1 || fullAccess[0].Id != tokenId)
            return false;
    ...
    {
        if (await WouldRemoveLastFullAccessTokenAsync(tokenId, newScopes: null, newEnvironments: environments))
        {
            Log.Warning("Refused to narrow environments on token {TokenId}: it is the last full-access token", tokenId);
            return false;
        }
    ...
    {
        if (await WouldRemoveLastFullAccessTokenAsync(tokenId, newScopes: scopes, newEnvironments: null))
        {
            Log.Warning("Refused to narrow scopes on token {TokenId}: it is the last full-access token", tokenId);
            return false;
        }
