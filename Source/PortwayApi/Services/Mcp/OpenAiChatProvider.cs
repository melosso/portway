...
public sealed class OpenAiChatProvider(string apiKey, string model, IHttpClientFactory httpFactory) : IChatProvider
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        catch (Exception ex) { Log.Error(ex, "OpenAI API request failed"); sendEx = ex; }

        if (sendEx is not null)
        {
            yield return new ChatDelta { Type = ChatDeltaType.Error, Delta = "Failed to reach OpenAI API." };
        ...
        if (!resp!.IsSuccessStatusCode)
        {
            ...
            Log.Error("OpenAI API error {Status}: {Body}", resp.StatusCode, err);
            yield return new ChatDelta { Type = ChatDeltaType.Error, Delta = $"OpenAI API error: {resp.StatusCode}" };
            yield return new ChatDelta { Type = ChatDeltaType.Done };
            yield break;
        }
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            catch (Exception ex) { Log.Debug(ex, "OpenAI: skipping unparseable SSE chunk"); continue; }
